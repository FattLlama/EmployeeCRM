...
if (!localStorage.getItem('employees3')) {
  const initialData = {
    managers: [],
    teamLeads: [],
    associates: []
  };
  localStorage.setItem('employees3', JSON.stringify(initialData));
}

// Function to display the chain of command
function displayChainOfCommand() {
  const employees = JSON.parse(localStorage.getItem('employees3'));
...
function hireEmployee() {
    ...
    if (name !== '') {
      ...
      localStorage.setItem('employees3', JSON.stringify(employees));
