...
if (!localStorage.getItem('employees2')) {
  const initialData = {
    managers: [],
    teamLeads: [],
    associates: []
  };
  localStorage.setItem('employees2', JSON.stringify(initialData));
}

// Function to display the chain of command
function displayChainOfCommand() {
  const employees = JSON.parse(localStorage.getItem('employees2'));
...
function hireEmployee() {
    ...
    if (name !== '') {
      ...
      localStorage.setItem('employees2', JSON.stringify(employees));
